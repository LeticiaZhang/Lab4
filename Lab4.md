...
![](Lab4_files/figure-gfm/unnamed-chunk-4-5.png)<!-- -->
...
This variable is normal, since the shapiro test results by group are not
significant, indicating that the data distribution within groups is
normal.
...
No, since the Levene’s test results are significant, indicating that the
variances across groups are not equal. \# Transformation

``` r
#if any of the assumption is not met, use transformation 

lab4data$Performance_log <- log10(lab4data$Performance)
lab4dataConG1$Performance_log <- log10(lab4dataConG1$Performance)
```
...
``` r
ggplot(lab4data, aes(x = Performance_log, y = Group)) + geom_violin() + theme_bruce()
```
